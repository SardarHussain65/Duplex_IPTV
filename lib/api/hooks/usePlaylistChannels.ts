 ...
 * Uses the REST API: GET /playlists/{id}/channels?page=...&limit=...&contentType=...&search=...&category=...
 * Search and category filtering are handled by the backend — pass a debounced query string to trigger a new fetch.
 */
export function usePlaylistChannels({
  ...
  return useInfiniteQuery({
    queryKey: ['playlist-channels', playlistId, limit, contentType, category, search ?? ''],
    queryFn: ({ pageParam = 1 }) => {
